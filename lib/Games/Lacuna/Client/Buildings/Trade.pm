package Games::Lacuna::Client::Buildings::Trade;
use 5.0080000;
use strict;
use warnings;
use Carp 'croak';
...
sub api_methods {
  return {
    ...
    get_waste_ships       => { default_args => [qw(session_id building_id)] },
    get_supply_ships              => { default_args => [qw(session_id building_id)] },
    view_supply_chains            => { default_args => [qw(session_id building_id)] },
    view_waste_chains             => { default_args => [qw(session_id building_id)] },
    create_supply_chain           => { default_args => [qw(session_id building_id)] },
    delete_supply_chain           => { default_args => [qw(session_id building_id)] },
    update_supply_chain           => { default_args => [qw(session_id building_id)] },
    update_waste_chain            => { default_args => [qw(session_id building_id)] },
    add_supply_ship_to_fleet      => { default_args => [qw(session_id building_id)] },
    add_waste_ship_to_fleet       => { default_args => [qw(session_id building_id)] },
    remove_supply_ship_from_fleet => { default_args => [qw(session_id building_id)] },
    remove_waste_ship_from_fleet  => { default_args => [qw(session_id building_id)] },
...
Games::Lacuna::Client::Buildings::Trade - The Trade Ministry building
