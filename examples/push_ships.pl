...
if (!@push_ships) {
    warn "No ships available to be pushed\n";
    exit;
}

if ( $min_push > scalar @push_ships ) {
    warn "Less than --min_push ships available to be pushed\n";
    exit;
}
...
By default is not set, so any trade ship above 50K is a candidate.
