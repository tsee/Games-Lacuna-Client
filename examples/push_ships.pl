...
# Find the TradeMin
  ...
  if ( $max_push && scalar @push_ships > $max_push) {
    splice @push_ships, $max_push;
  }
  ...
  for my $trade_ship (@trade_ships) {
    ...
    printf "Pushed %d: %s; ", scalar @push_ships, $push_ships[0]->{name};
...
MAX_PUSH is the maximum number of matching ships that will be pushed.

SHIP_NAME is the name of the ship to be used.  Partial matches are fine.
