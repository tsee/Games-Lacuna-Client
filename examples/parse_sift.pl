...
use utf8;
  ...
  my $total_g = 0;
  my $total_p = 0;
  for my $pname (sort keys %$idata) {
    printf "From: %s:%s\n", $pname, $idata->{$pname}{fid};
    for my $ship_id (sort keys %{$idata->{$pname}{ships}}) {
      print "$idata->{$pname}{ships}{$ship_id}{name} : $ship_id\n";
      my $pmax_length = max map { length $_->{name} } @{$idata->{$pname}{ships}{$ship_id}{plans}};
      my $gmax_length = max map { length $_->{name} } @{$idata->{$pname}{ships}{$ship_id}{glyphs}};

      my %plan_out;
      my $ship_p = 0;
      for my $plan (@{$idata->{$pname}{ships}{$ship_id}{plans}}) {
      ...
      my %glyph_out;
      my $ship_g = 0;
      for my $glyph (@{$idata->{$pname}{ships}{$ship_id}{glyphs}}) {
      ...
      printf "\nTotal of %d plans and %d glyphs on %s\n", $ship_p, $ship_g, $idata->{$pname}{ships}{$ship_id}{name};
      $total_p += $ship_p;
      $total_g += $ship_g;
    }
  }
